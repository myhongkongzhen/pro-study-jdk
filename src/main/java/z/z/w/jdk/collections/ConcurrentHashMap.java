...
/**************************************************************************
 * <pre>
 *     FileName: z.z.w.jdk.collections.ConcurrentHashMap
 *         Desc:
 *       author: Z_Z.W - myhongkongzhen@gmail.com
 *      version: 2015-12-23 16:26
 *   LastChange: 2015-12-23 16:26
 *      History:
 * </pre>
 **************************************************************************/
 ...
 * 一個哈希表支持的並發檢索與可調整的期望的並發更新
 * adjustable expected concurrency for updates. This class obeys the
 * same functional specification as {@link java.util.Hashtable}, and
 * 這個類服從相同的函數規範如同hashtable
 ...
 * 這個類內部操作的，用hashtable在程序中，依靠他的線程安全，而不是他的鎖細節
 ...
 * 簡單的，迭代與美劇返回的元素反射哈希表的狀態在某些電商或者從迭代或美劇的創建上
 ...
 * 這個表內部的位置嘗試允許不爭奪並發更新數量的說明
 * number of concurrent updates without contention. Because placement
 * in hash tables is essentially random, the actual concurrency will
 * 因為位置在哈希表中是實質上隨機的，真實的並發會變化
 * vary.  Ideally, you should choose a value to accommodate as many
 * 理想狀況下, 你應該選擇一個值，如同將要並發改變表一樣多的線程空間
 * threads as will ever concurrently modify the table. Using a
 * significantly higher value than you need can waste space and time,
 * 用一個標記更高的值比起你需要的浪費空間和時間
 * and a significantly lower value can lead to thread contention. But
 * 以及一個標記更低的值能夠領導線程爭奪資源
 * overestimates and underestimates within an order of magnitude do
 * 但高估或者低估沒有一個大小順序不能明顯的碰撞
 * not usually have much noticeable impact. A value of one is
 * appropriate when it is known that only one thread will modify and
 * 一個中的值是適當的，當他知道僅僅一條線程改變并且其他線程只讀
 * all others will only read. Also, resizing this or any other kind of
 * 童謠，重新大小這個貨哈希表的另一些類型
 * hash table is a relatively slow operation, so, when possible, it is
 * 是滿操作關係的，因此，黨可能的時候，
 * a good idea to provide estimates of expected table sizes in
 * 這是一個好的理念提供預估的期望的表大小在構造的時候
 * constructors.
 *
 * <p>This class and its views and iterators implement all of the
 * 這個類和他的視圖在迭代實現map以及iterator接口的所有可選方法
 ...
 * 如同hashtable但不想hashmap，這個類不允許null用於key或者value
        ...
        implements java.util.concurrent.ConcurrentMap<K, V>, Serializable
{
     ...
     * 他自身的誒個端都是一個並發的可讀的哈希表
     * reduce footprint, all but one segments are constructed only
     * 減少印記，所有的端都僅僅當首次需要的時候創建
     * when first needed (see ensureSegment). To maintain visibility
     * in the presence of lazy construction, accesses to segments as
     * 維護課件的延遲的構造存在，訪問端作為一個短表的元素
    ...
    final class KeyIterator
            ...
            implements java.util.Iterator<K>, Enumeration<K>
    {
    ...
    final class ValueIterator
            extends HashIterator
            implements java.util.Iterator<V>, Enumeration<V>
    {
