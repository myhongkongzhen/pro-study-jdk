...
/*********************************************************************************************
 * <pre>
 *     FileName: z.z.w.jdk.collections.AbstractMap
 *         Desc:
 *       author: Z_Z.W - myhongkongzhen@gmail.com
 *      version: 2015-12-15 16:53
 *   LastChange: 2015-12-15 16:53
 *      History:
 * </pre>
 *********************************************************************************************/
 ...
 * 實現一個不可改變的map，程序員需要擴展這個類
 * class and provide an implementation for the <tt>entrySet</tt> method, which
 * 并提供entrySet方法的實現
 * returns a set-view of the map's mappings.  Typically, the returned set
 * 由map影射的返回set的視圖
 * will, in turn, be implemented atop <tt>AbstractSet</tt>.  This set should
 * 通常的，返回的set事實上，實現與AbstractSet之上
 * not support the <tt>add</tt> or <tt>remove</tt> methods, and its iterator
 * 這個set不會支持add，remove方法
 * should not support the <tt>remove</tt> method.
 * 他的迭代器不支持remove方法
 *
 * <p>To implement a modifiable map, the programmer must additionally override
 * 實現一個可改變的map，程序員必須額外的複寫這個類的put方法
 * this class's <tt>put</tt> method (which otherwise throws an
 * 否則會拋出一個異常
 * <tt>UnsupportedOperationException</tt>), and the iterator returned by
 * <tt>entrySet().iterator()</tt> must additionally implement its
 * 並且迭代器由entrySet.iterator返回必須額外的實現remove方法
 * <tt>remove</tt> method.
 *
 * <p>The programmer should generally provide a void (no argument) and map
 * 程序眼通常的提供一個無參數的和map的構造器
 * constructor, as per the recommendation in the <tt>Map</tt> interface
 * 每一個建議在Map接口中特別指出
 * specification.
 *
 * <p>The documentation for each non-abstract method in this class describes its
 * 對於每個非抽象的方法在這個類中的文檔描述他的實現細節
